function buildHTMLTable({ availability, location, collection, callNumber, status }) {
	/**
	 * TODO3: After TODO1, all items should be displayed but not necessarily sorted by available or not. Seperate the sections
	 * example here: https://gailborden.bibliocommons.com/v2/availability/S151C3376400 by available and unavailable.
	 * 
	 * Head of table:  <tr class="cp-table-row group-heading-row">
		...
		which I can put in between these two lines: </thead>
    <tbody class="cp-table-body">
	 * 
	 * TODO4: <circle cx="6" cy="6" r="6" fill="#008000" fill-rule="evenodd"></circle> is the green circle svg for availble items. 
	 * Will need to create a seperate section for available and unavailable items first (TODO3).
	 */
...
//TODO2: run the main on every navigation event
//TODO1: Currently this works for all items that have only one copy. Need to add another loop to display table for all copies
async function main() {
	...
	for (let i = 0; i < results.length; i++) {
		...
		if (results[i].innerText.indexOf("View location availability") != -1) {
			...
			if (href != null) {
				...
				const bibItemsJSON = (await fetchJSON(newHref)).entities.bibItems;
				...
				var args = {
					availability: firstElementOfbibItemsJSON.availability.status,
